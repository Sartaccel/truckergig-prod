...
const Getaquote: React.FC = (props: any) => {
  ...
  useEffect(() => {
    axios
      .post(`${urls.baseUrl}services`, params)

      .then(function(response) {
        const data = response.data.data;
        if (response.status === 200 && data.length) {
          var check_orders = data.filter(
            (order) => order.id == router.query.serviceid
          );
  ...
  const allowOnlyLettersAndSpaces = (
  ...
  return (
    <>
      ...
      <div className="container">
        <div>
          <h1 className={`${styles["equipment-header-title"]}  `}>
            {selectedCategoryData && selectedCategoryData.name}
          </h1>
        </div>
        <div className="row">
          ...
          <div className="col-lg-7 col-9 pt-4">
            <form onSubmit={handleSubmit(onSubmitHandler)}>
              <div className="p-1">
                <p>
                  BUSINESS <strong>CONTACT DETAILS</strong>
                </p>
              </div>
              <div className="p-1">
                <div>
                  <div className={`${styles["equipment-form-text"]}`}>
                    <label>Business/Individual Name</label>
